    ...
    return(
      ...
      <div className={classes.terminal}>
          ...
          <InputBase autoFocus value={query} onChange={(e)=>setQuery(e.target.value)} className={classes.inputBase} onKeyDown={(e)=>{
            if(e.key==='Enter'){
              let newSearchQuery=(e.target.value).replace(' ','+')
              window.location.href=`/search?searchQuery=${newSearchQuery}`;
            }
          }}/>
        </div>
      <div className={classes.content}>
        <img className={classes.poster} src={movieDetails.poster ? movieDetails.poster : noDetailsImage }/>
        <div style={{textAlign:'start',padding:'8px'}}>
            ...
            <Typography className={classes.detailKeyText}>Plot: <span className={classes.detailValueText}>{movieDetails.plot && movieDetails.plot.length>210 ? movieDetails.plot.substr(0,210)+'...': movieDetails.plot }</span></Typography>
        </div>
      </div>
      <div>
      ...
      <div style={{margin:'15px 0px'}}>
          {
            movieDetails.downloadUrls && movieDetails.downloadUrls.length>0 ? movieDetails.downloadUrls.map((url)=>{
              return (
              <>
                <Typography style={{color:'white',fontFamily:'Ubuntu',textDecoration:'none'}}>
                  <a href={url}>
                    {url}
                  </a>
                </Typography>
...
export default dynamic(() => Promise.resolve(ContentDiv), {
  ssr: false,
})
